<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="故障照片" v-if="type!='cross' || (type=='cross' && !ro.guzhangzhaopian)" prop="guzhangzhaopian">
				<file-upload
					tip="点击上传故障照片"
					action="file/upload"
					:limit="3"
			...
			<el-form-item class="add-item" v-else label="故障照片" prop="guzhangzhaopian">
				<img v-if="ruleForm.guzhangzhaopian.substring(0,4)=='http'" class="upload-img" v-bind:key="index" :src="ruleForm.guzhangzhaopian.split(',')[0]">
				<img v-else class="upload-img" v-bind:key="index" v-for="(item,index) in ruleForm.guzhangzhaopian.split(',')" :src="baseUrl+item">
			</el-form-item>
			<el-form-item class="add-item" label="设备位置" prop="shebeiweizhi">
			...
			<el-form-item class="add-item" label="社区账号" prop="shequzhanghao">
				<el-select  @change="shequzhanghaoChange" v-model="ruleForm.shequzhanghao" placeholder="请选择社区账号">
					<el-option
	...
	export default {
		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='guzhangzhaopian'){
							this.ruleForm.guzhangzhaopian = obj[o].split(",")[0];
				...
				this.$http.get('option/shequrenyuan/shequzhanghao', {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
						this.shequzhanghaoOptions = res.data.data;
					}
				});
			...
			// 下二随
			shequzhanghaoChange () {
				this.$http.get('follow/shequrenyuan/shequzhanghao?columnValue=' + this.ruleForm.shequzhanghao, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
						if(res.data.data.renyuanming){
							this.ruleForm.renyuanming = res.data.data.renyuanming
						}
					}
				});
			},
			...
			// 多级联动参数
			info() {
				this.$http.get(`baoxiurenwu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`baoxiurenwu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
			...
			guzhangzhaopianUploadChange(fileUrls) {
				this.ruleForm.guzhangzhaopian = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");
			},
		}
	};
</script>
