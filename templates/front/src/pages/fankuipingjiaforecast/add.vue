	...
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="维修时长\小时" prop="weixiushichang">
				<el-input v-model="ruleForm.weixiushichang" 
			...
			<el-form-item class="add-item" label="满意度" prop="score">
				<el-input v-model="ruleForm.score" 
					placeholder="满意度" clearable :disabled=" false  ||ro.score"></el-input>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`fankuipingjiaforecast/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`fankuipingjiaforecast/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
