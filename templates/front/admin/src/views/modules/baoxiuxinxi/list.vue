<template>
	<div class="main-content" :style='{"padding":"0","fontSize":"15px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"padding":"20px 30px","margin":"10px 0 0","borderRadius":"4px 4px 0 0","flexWrap":"wrap","background":"#fff","display":"flex"}'>
					...
					<download-excel v-if="isAuth('baoxiuxinxi','导出')" class="export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "报修信息.xls">
						<!-- 导出excel -->
					...
					<el-button class="btn18" v-if="isAuth('baoxiuxinxi','报修进度统计')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						报修进度统计
					</el-button>
				</el-row>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('baoxiuxinxi','分配')" @click="baoxiurenwuCrossAddOrUpdateHandler(scope.row,'cross','','','qingqiujindu','已处理','已处理,未处理'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								分配
							</el-button>
							<el-button class="edit" v-if=" isAuth('baoxiuxinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>
		...
		<baoxiurenwu-cross-add-or-update v-if="baoxiurenwuCrossAddOrUpdateFlag" :parent="this" ref="baoxiurenwuCrossaddOrUpdate"></baoxiurenwu-cross-add-or-update>
		...
		<el-dialog
			...
			width="800">
			<div id="qingqiujinduChart1" style="width:100%;height:600px;"></div>
...
<script>
	...
	import baoxiurenwuCrossAddOrUpdate from "../baoxiurenwu/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				baoxiurenwuCrossAddOrUpdateFlag: false,
				layouts: ["total","prev","pager","next","sizes","jumper"],
//导出excel
				json_fields: {
					"报修编号": "bianhao",    //常规字段
					"设备名称": "shebeiming",    //常规字段
					"设备类型": "shebeileixing",    //常规字段
					"设备位置": "shebeiweizhi",    //常规字段
					"故障照片": "guzhangzhaopian",    //常规字段
					"紧急程度": "jinjichengdu",    //常规字段
					"故障描述": "guzhangmiaoshu",    //常规字段
					"账号": "zhanghao",    //常规字段
					"姓名": "fullname",    //常规字段
					"报修时间": "baoxiushijian",    //常规字段
					"请求进度": "qingqiujindu",    //常规字段
				},
				json_meta: [
					[
						{
		...
		methods: {
			...
			baoxiurenwuCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.baoxiurenwuCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.baoxiurenwuCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.baoxiurenwuCrossaddOrUpdate.init(row.id,type);
				});
			},
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					var qingqiujinduChart1 = echarts.init(document.getElementById("qingqiujinduChart1"),'macarons');
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							qingqiujinduChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								qingqiujinduChart1.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			getDataList() {
				...
				if(this.searchForm.qingqiujindu!='' && this.searchForm.qingqiujindu!=undefined){
					params['qingqiujindu'] = this.searchForm.qingqiujindu
				}
