	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="select" v-if="type!='info'" label="社区账号" prop="shequzhanghao" >
					<el-select :disabled="ro.shequzhanghao" @change="shequzhanghaoChange" v-model="ruleForm.shequzhanghao" placeholder="请选择社区账号">
						<el-option
							v-for="(item,index) in shequzhanghaoOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item class="input" v-else-if="ruleForm.shequzhanghao" label="社区账号" prop="shequzhanghao" >
	...
	export default {
		...
		methods: {
			...
			init(id,type) {
				...
				this.$http({
					url: `option/shequrenyuan/shequzhanghao`,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
						this.shequzhanghaoOptions = data.data;
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			shequzhanghaoChange () {
				this.$http({
					url: `follow/shequrenyuan/shequzhanghao?columnValue=`+ this.ruleForm.shequzhanghao,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
						if(data.data.renyuanming){
							this.ruleForm.renyuanming = data.data.renyuanming
						}
					} else {
						this.$message.error(data.msg);
					}
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `baoxiurenwu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.baoxiurenwuCrossAddOrUpdateFlag = false;
											this.parent.search();
