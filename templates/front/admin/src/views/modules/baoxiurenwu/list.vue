<template>
	<div class="main-content" :style='{"padding":"0","fontSize":"15px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","borderRadius":"4px","background":"#fff","display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						...
						<el-input v-model="searchForm.shebeiming" placeholder="设备名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
				...
				<el-row class="actions" :style='{"padding":"20px 30px","margin":"10px 0 0","borderRadius":"4px 4px 0 0","flexWrap":"wrap","background":"#fff","display":"flex"}'>
					<el-button class="add" v-if="isAuth('baoxiurenwu','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('baoxiurenwu','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"","height":"34px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('baoxiurenwu','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
			...
			<div :style='{"width":"100%","padding":"0 30px","backgroundColor":"#fff","fontSize":"14px","color":"#000"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column  :resizable='true' prop="guzhangzhaopian" width="200" label="故障照片">
						<template slot-scope="scope">
							<div v-if="scope.row.guzhangzhaopian">
								<img v-if="scope.row.guzhangzhaopian.substring(0,4)=='http'&&scope.row.guzhangzhaopian.split(',w').length>1" :src="scope.row.guzhangzhaopian" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.guzhangzhaopian)">
								<img v-else-if="scope.row.guzhangzhaopian.substring(0,4)=='http'" :src="scope.row.guzhangzhaopian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.guzhangzhaopian.split(',')[0])">
								<img v-else :src="$base.url+scope.row.guzhangzhaopian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.guzhangzhaopian.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('baoxiurenwu','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('baoxiurenwu','评价')" @click="fankuipingjiaCrossAddOrUpdateHandler(scope.row,'cross','是','','[1]','已反馈')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								评价
							</el-button>
							<el-button class="edit" v-if=" isAuth('baoxiurenwu','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>
							...
							<el-button class="del" v-if="isAuth('baoxiurenwu','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		<fankuipingjia-cross-add-or-update v-if="fankuipingjiaCrossAddOrUpdateFlag" :parent="this" ref="fankuipingjiaCrossaddOrUpdate"></fankuipingjia-cross-add-or-update>
...
<script>
	...
	import fankuipingjiaCrossAddOrUpdate from "../fankuipingjia/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				fankuipingjiaCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			fankuipingjiaCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.fankuipingjiaCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','baoxiurenwu');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.fankuipingjiaCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.fankuipingjiaCrossaddOrUpdate.init(row.id,type);
				});
			},
			...
			getDataList() {
				...
				let params = {
 				...
 				if(this.searchForm.shebeiming!='' && this.searchForm.shebeiming!=undefined){
					params['shebeiming'] = '%' + this.searchForm.shebeiming + '%'
				}
				...
				this.$refs["shBatchForm"].validate(valid => {
					if(valid){
						this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
							...
							this.$http({
								url: "baoxiurenwu/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
								method: "post",
								data: this.batchIds
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
